...
void graphics::Material::Bind(graphics::MaterialGLBinding binding) {
    if (binding.diffuse_texture.uniform_has_texture >= 0) {
        glUniform1i(binding.diffuse_texture.uniform_has_texture, Diffuse_Texture != NULL ? 1 : 0);
    }

    if (binding.toon_texture.uniform_has_texture >= 0) {
        glUniform1i(binding.toon_texture.uniform_has_texture, Toon_Texture != NULL ? 1 : 0);
    }

    if (binding.illumination_texture.uniform_has_texture >= 0) {
        glUniform1i(binding.illumination_texture.uniform_has_texture, Illumination_Texture!= NULL ? 1 : 0);
    }
    ...
    if (binding.uniform_uses_lighting >= 0) {
        glUniform1i(binding.uniform_uses_lighting, is_lit);
    }

    if (binding.uniform_uses_colored_vertices >= 0) {
        glUniform1i(binding.uniform_uses_colored_vertices, is_colored);
    }
